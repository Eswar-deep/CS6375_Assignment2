...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    for b in range(num_boxes):
        pred_cx = output[:, 5 * b + 0, :, :]  # predicted center x
        pred_cy = output[:, 5 * b + 1, :, :]  # predicted center y
        pred_w  = output[:, 5 * b + 2, :, :]  # predicted width
        pred_h  = output[:, 5 * b + 3, :, :]  # predicted height
        pred_conf = output[:, 5 * b + 4, :, :]  # predicted confidence

        # Box assignment mask for this box index
        mask = box_mask[:, b, :, :]  # shape: (batch_size, 7, 7)
        iou_target = box_confidence[:, b, :, :]  # same shape

        # Coordinate losses (x, y, w, h)
        loss_x += weight_coord * torch.sum(mask * (pred_cx - gt_box[:, 0, :, :]) ** 2)
        loss_y += weight_coord * torch.sum(mask * (pred_cy - gt_box[:, 1, :, :]) ** 2)

        # Add epsilon to avoid sqrt of 0
        eps = 1e-6
        loss_w += weight_coord * torch.sum(mask * (torch.sqrt(pred_w + eps) - torch.sqrt(gt_box[:, 2, :, :] + eps)) ** 2)
        loss_h += weight_coord * torch.sum(mask * (torch.sqrt(pred_h + eps) - torch.sqrt(gt_box[:, 3, :, :] + eps)) ** 2)

        # No-object loss
        loss_noobj += weight_noobj * torch.sum((1.0 - mask) * (pred_conf ** 2))

    # Classification loss
    # gt_box[:, 4, :, :] should be 1 if object is present, so we expand this into shape (batch, 1, 7, 7)
    gt_class = gt_box[:, 4:, :, :]  # shape: (batch_size, 1, 7, 7)
    pred_class = output[:, 5 * num_boxes:, :, :]  # shape: (batch_size, 1, 7, 7)
    loss_cls = torch.sum(gt_mask.unsqueeze(1) * (gt_class - pred_class) ** 2)
